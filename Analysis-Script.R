...
df_table_networks$Conditions <- factor(df_table_networks$Conditions,
                                       levels=c("Homophilous Centralized","Diverse Centralized","Homophilous Egalitarian",
                                                "Diverse Egalitarian","Homophilous Control" ,"Diverse Control"))

levels(df_table_networks$Conditions) <- c("Homo.\n Cent.","Diverse\n Cent.","Homo.\n Egal.",
                                          "Diverse\n Egal.","Homo.\n Control" ,"Diverse\n Control")
...
ggplot(df_summary, aes(x = Conditions, y = grand_mean_ind, fill = Conditions)) +
  geom_bar(stat = "identity", position = "dodge", color='black') +
  geom_hline(yintercept = 0, color = 'black') + 
  ...
  theme(axis.text=element_text(size=10), 
        ...
        axis.text.x=element_text(size=10), 
        panel.border = element_rect(colour = "black", fill=NA, size=1)) + 
  geom_segment(aes(x = 4.65, xend = 4.65, y = -300, yend = -700),
               lineend = 'round', linejoin = 'round',
               size = 1,
               arrow = arrow(type = "closed", length = unit(.1, "inches"))) +
  geom_text(aes(x = 4.65, y = -775, label='Improves'), size = 5) +
  geom_segment(aes(x = 0.35, xend = 0.35, y = 700, yend = 1200),
               lineend = 'round', linejoin = 'round',
               size = 1,
               arrow = arrow(type = "closed", length = unit(.1, "inches"))) +
  geom_text(aes(x = 0.35, y = 1325, label='Worsens'), size = 5) +
